...
Ce document contient un enregistrement du code des fichiers clés qui composent le système d'authentification et d'inscription fonctionnel de l'application.

---

## 1. Options d'Authentification (`src/lib/auth-options.ts`)

Ce fichier définit la stratégie d'authentification, les fournisseurs (`Credentials` et `Google`), les callbacks pour gérer les sessions et les tokens JWT, ainsi que l'intégration avec Prisma. Il inclut la logique pour assigner des rôles et des statuts de validation.
...
## 2. Route API NextAuth (`src/app/api/auth/[...nextauth]/route.ts`)

Ce fichier est le point d'entrée pour toutes les requêtes d'authentification gérées par NextAuth.js (connexion, déconnexion, session, etc.). Il est standard et ne change pas.
...
Ce composant React gère l'interface de connexion. Il inclut les champs pour l'email/mot de passe et un bouton pour se connecter avec Google. Il gère également la redirection après une connexion réussie en fonction du rôle et du statut de validation de l'utilisateur.
...
## 4. Route API d'Inscription (`src/app/api/auth/register/route.ts`)

Cette nouvelle route API gère la création de nouveaux utilisateurs via le formulaire d'inscription. Elle vérifie si l'utilisateur existe déjà, hache le mot de passe, et crée le nouvel utilisateur avec le rôle et le statut de validation appropriés.
...
## 5. Types d'Authentification (`src/types/next-auth.d.ts`)

Ce fichier étend les types par défaut de NextAuth.js pour inclure des champs personnalisés dans la session et le token JWT, comme l'ID de l'utilisateur, son rôle, l'ID de sa classe et son statut de validation.
