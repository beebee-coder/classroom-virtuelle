...
export async function POST(request: NextRequest) {
  const operation: WhiteboardOperation = await request.json();
  const sessionId = operation.sessionId;

  if (!sessionId) {
    return NextResponse.json({ error: 'SessionId manquant dans l\'opération' }, { status: 400 });
  }

  console.warn(`[API-WB-OPS] Appel d'une route dépréciée. Redirection de la logique vers /api/session/${sessionId}/sync`);
