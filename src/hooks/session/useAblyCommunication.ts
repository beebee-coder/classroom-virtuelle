...
export function useAblyCommunication({ 
  ...
  // Utilisation de useRef pour garder une référence stable aux callbacks
  ...
  useEffect(() => {
    ...
    if (isTimerRunning && timerTimeLeft > 0) {
      timerInterval = setInterval(() => setTimerTimeLeft((prevTime) => Math.max(0, prevTime - 1)), 1000);
    }
    return () => { if (timerInterval) clearInterval(timerInterval); };
  }, [isTimerRunning, timerTimeLeft]);

  // Callbacks pour les événements Ably
  const handleSignalEvent = useCallback((message: AblyTypes.Message) => {
    if (message.data.target === currentUserId) {
      onSignalReceivedRef.current(message.data.userId, message.data.signal);
    }
  }, [currentUserId]);
  ...
  const handleActiveToolChangeEvent = useCallback((message: AblyTypes.Message) => setActiveTool(message.data.tool), [setActiveTool]);
  ...
  const handleTimerResetEvent = useCallback((message: AblyTypes.Message) => { setTimerTimeLeft(message.data.duration); setIsTimerRunning(false); }, []);
  const handleQuizStartedEvent = useCallback((message: AblyTypes.Message) => { setActiveTool('quiz'); setActiveQuiz(message.data.quiz as Quiz); }, [setActiveQuiz, setActiveTool]);
  ...
  const handleBreakoutRoomsStarted = useCallback((message: AblyTypes.Message) => {
    ...
    setBreakoutRoomInfo(myRoom || null);
  }, [currentUserId]);

  const handleBreakoutRoomsEnded = useCallback(() => setBreakoutRoomInfo(null), []);
  ...
  useEffect(() => {
    ...
    ablyClient, isAblyConnected, sessionId, handlePresenceUpdate, handleSignalEvent,
    ...
    handleDocumentSharedEvent, handleWhiteboardControllerUpdateEvent, handleTimerStartedEvent,
    handleTimerPausedEvent, handleTimerResetEvent, handleQuizStartedEvent, handleQuizResponseEvent,
    handleQuizEndedEvent, handleQuizClosedEvent, handleBreakoutRoomsStarted, handleBreakoutRoomsEnded
  ]);
