        ...
        const interval = setInterval(() => {
            ...
            remoteStreams.forEach((stream, userId) => {
                ...
                if (peerState && now - peerState.lastStreamCheck > WEBRTC_CONFIG.STREAM_CHECK_INTERVAL) {
                    ...
                    // ✅ CORRECTION AMÉLIORÉE : Vérification plus robuste des streams
                    const isStreamActive = stream.active;
                    const hasActiveVideoTracks = stream.getVideoTracks().some(track => 
                        track.readyState === 'live' && track.enabled
                    );
                    const hasActiveAudioTracks = stream.getAudioTracks().some(track => 
                        track.readyState === 'live' && track.enabled
                    );
                    
                    // ✅ CORRECTION : Ne pas nettoyer si au moins une piste est active
                    const hasActiveTracks = hasActiveVideoTracks || hasActiveAudioTracks;
                    
                    if (!isStreamActive || !hasActiveTracks) {
